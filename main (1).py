"""
Project done by: Subhan Attique 20F-1121 && Hamza Mushtaq 20F-1119
"""
...
from simmulated_annealing import setValue
from best_first_search import Graph_best_first_search
...
class Ui_AISearchingTechniquesMainWindow(object):
    ...
    graphbffs = Graph_best_first_search(directed=False)
    graphucsD = Graph_ucs(directed=True)
    graphidsD = Graph_ids(directed=True)
    graphdlsD = Graph_dls(directed=True)
    graphbdsD = Graph_bds(directed=True)
    graphbffsD = Graph_best_first_search(directed=True)
        ...
        with open('test.txt', 'w') as f:
            ...
            if (graphType == "Undirectd Graph"):
                if searchType == "BFS":
                    ...
                    for i in range(0, self.counter):
                        self.graphucs.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphucs.uniform_cost_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        Graph_ucs.print_path(traced_path5, goal)
                        print('\nCost evaluated', cost3)

                elif searchType == "IDS":
                    for i in range(0, self.counter):
                        self.graphids.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphids.iterative_deepening_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        Graph_ids.print_path(traced_path5, goal)
                        print('\nLimit on which goal is found is::', cost3)

                elif searchType == "DLS":
                    for i in range(0, self.counter):
                        self.graphdls.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphdls.depth_limited_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        print('\nLimit on which goal is found is::', cost3)
                    ...
                    for i in range(0, self.counter):
                        self.graphbds.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphbds.bidirectional_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        Graph_bds.print_path(traced_path5, goal)
                        print('\nLimit on which goal is found is::', cost3)

                elif searchType == "BestFirstSearch":
                    for i in range(0, self.counter):
                        self.graphbffs.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphbffs.best_first_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        Graph_best_first_search.print_path(traced_path5, goal)
                        print('\nLimit on which goal is found is::', cost3)

                elif searchType == "Simmulated_Annealing":
                    setValue(passList)
                ...
                if searchType == "BFS":
                    ...
                    if (traced_path5):
                        print('Path:', end=' ')
                        Graph_astar.print_path(traced_path5, cost3, goal)
                        print('\nCost:', cost3)

                elif searchType == "UCS":
                    for i in range(0, self.counter):
                        self.graphucsD.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphucsD.uniform_cost_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        Graph_ucs.print_path(traced_path5, goal)
                        print('\nCost evaluated', cost3)
                    ...
                    for i in range(0, self.counter):
                        self.graphidsD.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphidsD.iterative_deepening_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        Graph_ids.print_path(traced_path5, goal)
                        print('\nLimit on which goal is found is::', cost3)
                    ...
                    for i in range(0, self.counter):
                        self.graphdlsD.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphdlsD.depth_limited_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        print('\nLimit on which goal is found is::', cost3)
                    ...
                    for i in range(0, self.counter):
                        self.graphbdsD.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphbdsD.bidirectional_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        Graph_bds.print_path(traced_path5, goal)
                        print('\nLimit on which goal is found is::', cost3)
                    ...
                    for i in range(0, self.counter):
                        self.graphbffsD.add_edge(Node1_arr[i], Node2_arr[i], int(self.EdgeWeight_arr[i]))
                    start = self.StartNode_input.text()
                    goals = Goal_list
                    traced_path5, cost3, goal = self.graphbffsD.best_first_search(start, goals)
                    if (traced_path5):
                        print('Path:', end=' ')
                        Graph_best_first_search.print_path(traced_path5, goal)
                        print('\nLimit on which goal is found is::', cost3)
    ...
    def HeuristicPushed(self):
        ...
        self.graphastar.set_heuristics(self.HeuristicDict)
        self.graphastarD.set_heuristics(self.HeuristicDict)
        self.Node_Input.clear()
        self.NodeHeuristic_input.clear()

    def HeuristicPushedbfs(self):
        InputHeuristic = int(self.NodeHeuristic_input.text())
        InputNodeH = self.Node_Input.text()
        self.H[self.Node_Input.text()] = self.NodeHeuristic_input.text()
        self.HeuristicDict.update({InputNodeH: InputHeuristic})
        self.graphbffs.set_heuristics(self.HeuristicDict)
        self.Node_Input.clear()
        self.NodeHeuristic_input.clear()
    ...
    def retranslateUi(self, AISearchingTechniquesMainWindow):
        ...
        self.SearchTypecomboBox.setItemText(2, _translate("AISearchingTechniquesMainWindow", "UCS"))
        self.SearchTypecomboBox.setItemText(3, _translate("AISearchingTechniquesMainWindow", "IDS"))
        self.SearchTypecomboBox.setItemText(4, _translate("AISearchingTechniquesMainWindow", "DLS"))
        self.SearchTypecomboBox.setItemText(5, _translate("AISearchingTechniquesMainWindow", "BDS"))
        self.SearchTypecomboBox.setItemText(6, _translate("AISearchingTechniquesMainWindow", "Simmulated_Annealing"))
        self.SearchTypecomboBox.setItemText(7, _translate("AISearchingTechniquesMainWindow", "BestFirstSearch"))
        self.SearchTypecomboBox.setItemText(8, _translate("AISearchingTechniquesMainWindow", "A*"))
